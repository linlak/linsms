    ...
    p.ProcessLinkOrImageOrFootnote = function () {
        ...
        if (this.m_Markdown.ExtraMode && token_type == TokenType_link && p.SkipChar('^')) {
            ...
            if (id != null && p.SkipChar(']')) {
                ...
                if (footnote_index >= 0) {
                    // Yes it's a footnote
                    return this.CreateDataToken(TokenType_footnote, {
...
var MarkdownDeepEditor = new function () {
    ...
    function Editor(textarea, div_html) {
        ...
        BindEvent(textarea, "mousedown", function () {
            ed.SetUndoMode(undomode_navigate);
        });
    ...
    pub.cmd_img = function (ctx) {
        ctx.TrimSelection();
        return this.cmd_image(ctx);
    }
    pub.cmd_image = function (ctx, url = false) {
        if (!!url) {
            ctx.TrimSelection();
        }
    ...
    pub.InvokeCommand = function (id, param = false) {
        ...
        let res = false;
        if (!!param) {
            res = this["cmd_" + id](state, param);
        } else {
            res = this["cmd_" + id](state);
        }
        if (res) {
