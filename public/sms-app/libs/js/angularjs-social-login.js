...
socialLogin.provider("social", function () {
    var fbKey, fbApiV, googleKey, linkedInKey, linkedInDomain;
    return {
        ...
        setLinkedInKey: function (value, value1) {
            linkedInKey = value;
            linkedInDomain = value1;
            var lIN, d = document,
                ref = d.getElementsByTagName('script')[0];
    ...
    function ($window, $rootScope) {
        ...
        return {
            logout: function () {
                ...
                console.log('_login_provider', provider);
                switch (provider) {
                    case "google":
...
socialLogin.directive("fbLike", [function () {
    return ({
        restrict: "AE",
        replace: true,
        scope: {
        ...
        link: function (scope, elem, attr) {
            function renderLikeButton() {
                var likeBtnHtml = '<div class="fb-like" data-href="' + scope.url +
                    '" data-layout="' + scope.layout + '" data-action="' + scope.actionType +
                    '" data-show-faces="' + scope.dataShowFaces + '"';
                if (angular.isDefined(scope.width) && scope.width !== null && parseInt(scope.width) > 0) {
                    likeBtnHtml += ' data-width="' + parseInt(scope.width) + '"';
                }
                likeBtnHtml += '></div>';
                elem.html(likeBtnHtml);
                FB.XFBML.parse(elem.parent()[0]);
            }
            renderLikeButton();
        }
    });
